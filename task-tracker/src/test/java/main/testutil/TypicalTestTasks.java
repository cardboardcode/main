...
public class TypicalTestTasks {
    ...
    public TypicalTestTasks() {
        try {
            ...
            deadline1 = new TaskBuilder().withMessage("return books").withDate(TestUtil.setDate(2016, 11, 10, 12, 00)).withPriority(PriorityType.HIGH).build();
            deadline2 = new TaskBuilder().withMessage("buy groceries").withDate(TestUtil.setDate(2016, 10, 31, 9, 30)).withPriority(PriorityType.NORMAL).build();
            deadline3 = new TaskBuilder().withMessage("finish coding").withDate(TestUtil.setDate(2016, 11, 7, 11, 00)).withPriority(PriorityType.NORMAL).build();

            event1= new TaskBuilder().withMessage("meetup with friends").withDates(TestUtil.setDate(2016, 11, 20, 11, 00), TestUtil.setDate(2016, 11, 20, 21, 00)).withPriority(PriorityType.NORMAL).build();
            event2= new TaskBuilder().withMessage("interview").withDates(TestUtil.setDate(2016, 12, 2, 15, 00), TestUtil.setDate(2016, 12, 2, 15, 30)).withPriority(PriorityType.HIGH).build();
            event3= new TaskBuilder().withMessage("competition").withDates(TestUtil.setDate(2016, 12, 15, 8, 00), TestUtil.setDate(2016, 12, 21, 18, 00)).withPriority(PriorityType.HIGH).build();
